...
object CoffeeHouse {
  ...
  case class Status(guestCounter: Int)
...
class CoffeeHouse(caffeineLimit: Int) extends Actor with ActorLogging{
  ...
  override def supervisorStrategy: SupervisorStrategy = {
    val decider: SupervisorStrategy.Decider = {
      ...
      case Waiter.FrustratedException(correctCoffee, guest) =>
        barista.forward(Barista.PrepareCoffee(correctCoffee,guest))
  ...
  protected def createBarista: ActorRef =
    context.actorOf(
      ...
      // or Barista.props(prepareCoffeeDuration, baristaAccuracy).withRouter(FromConfig()),
      "barista"
    )
