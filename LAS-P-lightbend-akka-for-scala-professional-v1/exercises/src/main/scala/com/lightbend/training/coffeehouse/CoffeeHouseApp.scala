...
object CoffeeHouseApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val statusTimeout: Timeout =
      system.settings.config.getDuration("coffee-house.status-timeout", TimeUnit.MILLISECONDS).millis
...
class CoffeeHouseApp(system: ActorSystem)(implicit statusTimeout:Timeout) extends Terminal {
  ...
  protected def status(): Unit = {
    (coffeeHouse ? CoffeeHouse.GetStatus).mapTo[CoffeeHouse.Status] onComplete {
      case Success(status) => log.info(s"Status: guest count = ${status.guestCounter}")
      case Failure(error) => log.error(error, "Can't get guest count")
    }
  }
}
